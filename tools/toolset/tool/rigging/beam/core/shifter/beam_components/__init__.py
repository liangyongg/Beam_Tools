...
import rigging.beam
from rigging.beam.core import primitive, vector, transform
from rigging.beam.core import attribute, applyop, node, icon
...
class Main(object):
    ...
    def initialHierarchy(self):
        ...
        """
        # Root
        self.root = primitive.addTransformFromPos(self.model, self.getName("root"), self.guide.pos["root"])
    ...
    def addJoint(self,
        ...
        if self.options["joint_rig"]:
            ...
            jnt = primitive.addJoint(self.active_jnt, self.getName(str(name) + "_jnt"), transform.getTransform(obj))
            ...
            if gearMulMatrix:
                mulmat_node = applyop.gear_mulmatrix_op(obj + ".worldMatrix", jnt + ".parentInverseMatrix")
                dm_node = node.createDecomposeMatrixNode(mulmat_node + ".output")
                m = mulmat_node.attr('output').get()
            else:
                mulmat_node = node.createMultMatrixNode(obj + ".worldMatrix", jnt + ".parentInverseMatrix")
    ...
    def getNormalFromPos(self, pos):
        ...
        """
        if len(pos) < 3:
            rigging.beam.log("%s : Not enough references to define normal" %self.fullName, rigging.beam.sev_error)
    ...
    def getBiNormalFromPos(self, pos):
        ...
        """
        if len(pos) < 3:
            rigging.beam.log("%s : Not enough references to define binormal" %self.fullName, rigging.beam.sev_error)
    ...
    def getRelation(self, name):
        ...
        if name not in self.relatives.keys():
            rigging.beam.log("Can't find reference for object : " +self.fullName + "." + name, rigging.beam.sev_error)
            return False
    ...
    def getControlRelation(self, name):
        ...
        if name not in self.controlRelatives.keys():
            rigging.beam.log("Control tag relative: Can't find reference for "
                      " object : " + self.fullName + "." + name,rigging.beam.sev_error)
            return False
    ...
    def initControlTag(self):
        ...
        Also tells maya how to evaluate the control properly on parallel
        evaluation
