...
from . import guide,beam_components
from rigging.beam.core import primitive, attribute, skin, dag, icon, node

SHIFTER_COMPONENT_ENV_KEY = "BEAM_CORE_COMPONENT_PATH"

def getComponentDirectories():
    """Get the components directory"""
    return rigging.beam.core.utils.gatherCustomModuleDirectories(SHIFTER_COMPONENT_ENV_KEY,
                                                                 os.path.join(os.path.dirname(__file__), "beam_components"))

def importComponentGuide(comp_type):
    """Import the Component guide"""
    dirs = getComponentDirectories()
    defFmt = "rigging.beam.core.shifter.beam_components.{}.guide"
    customFmt = "{}.guide"

    module = rigging.beam.core.utils.importFromStandardOrCustomDirectories(dirs, defFmt, customFmt, comp_type)
    print "importComponentGuide.module",module
    return module

def importComponent(comp_type):
    """Import the Component """
    dirs = getComponentDirectories()
    defFmt = "rigging.beam.core.shifter.beam_components.{}"
    customFmt = "{}"
...
class Rig(object):
    ...
    def build(self):
        ...
        self.customStepDic["beamRun"] = self
        ...
        if customSteps:
            for step in customSteps:
                if not self.stopBuild:
                    if step.startswith("*"):
                        continue
                    self.stopBuild = guide.helperSlots.runStep(step.split("|")[-1][1:], self.customStepDic)
                else:
                    pm.displayWarning("Build Stopped")
                    break

    def preCustomStep(self, selection):
        if (selection[0].hasAttr("ismodel") and selection[0].attr("doPreCustomStep").get()):
    ...
    def initialHierarchy(self):
        ...
        """
        rigging.beam.log("Initial Hierarchy")
        ...
        self.rigName_att = attribute.addAttribute(self.model, "rig_name", "string", self.options["rig_name"])
        self.user_att = attribute.addAttribute(self.model, "user", "string", getpass.getuser())
        self.isWip_att = attribute.addAttribute(self.model, "wip", "bool", self.options["mode"] != 0)
        self.date_att = attribute.addAttribute(self.model, "date", "string", str(datetime.datetime.now()))
        self.mayaVersion_att = attribute.addAttribute(self.model, "maya_version", "string",str(pm.mel.eval("getApplicationVersionAsFloat")))
        self.gearVersion_att = attribute.addAttribute(self.model, "gear_version", "string", rigging.beam.getVersion())
        self.synoptic_att = attribute.addAttribute(self.model, "synoptic", "string", str(self.options["synoptic"]))
        self.comments_att = attribute.addAttribute(self.model, "comments", "string", str(self.options["comments"]))
        self.ctlVis_att = attribute.addAttribute(self.model, "ctl_vis", "bool", True)
        if versions.current() >= 201650:
            self.ctlVisPlayback_att = attribute.addAttribute(self.model, "ctl_vis_on_playback", "bool", True)
        ...
        if versions.current() >= 201650:
            pm.connectAttr(self.ctlVisPlayback_att,self.global_ctl.attr("hideOnPlayback"))
    ...
    def processComponents(self):
        ...
        for comp in self.guide.componentsIndex:
            ...
            rigging.beam.log("Init : " + guide_.fullName + " (" + guide_.type + ")")

            module = importComponent(guide_.type)
            print "rigging.beam.core.shifter.Rig.processComponents.module: ",module
            ...
            if comp.fullName not in self.componentsIndex:
                ...
                self.components_infos[comp.fullName] = [guide_.compType, guide_.getVersion(), guide_.author]

        # Creation steps
        self.steps = beam_components.Main.steps
        for i, name in enumerate(self.steps):
            ...
            for compName in self.componentsIndex:
                ...
                rigging.beam.log(name + " : " + comp.fullName +  " (" + comp.type + ")")
                comp.stepMethods[i]()
