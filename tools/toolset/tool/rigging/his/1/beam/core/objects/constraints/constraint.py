...
from rigging.beam.core.maths.mat44 import Mat44
...
class Constraint(SceneItem):
    ...
    def compute(self):
        ...
        bs.loadExtension('KrakenForCanvas')
        rtVal = bs.rtVal('KrakenForCanvas::Kraken%s' % cls)

        for c in self._constrainers:
            rtVal.addConstrainer('', bs.rtVal('Xfo', c.globalXfo).toMat44('Mat44'))

        # Using globalXfo here would cause a recursion
        return Xfo(rtVal.compute("Xfo",
                                 bs.rtVal('Xfo', self._constrainee.xfo).toMat44('Mat44')))
    ...
    def computeOffset(self):
        ...
        rtVal.offset = bs.rtVal('Mat44', Mat44())
        for c in self._constrainers:
            rtVal.addConstrainer('', bs.rtVal('Xfo', c.globalXfo).toMat44('Mat44'))
