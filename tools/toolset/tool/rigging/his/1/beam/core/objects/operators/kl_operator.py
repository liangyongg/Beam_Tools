...
from rigging.beam.core.maths import MathObject, Mat44, Xfo, Vec2, Vec3
from rigging.beam.core.objects.object_3d import Object3D
from rigging.beam.core.objects.operators.operator import Operator
...
class KLOperator(Operator):
    ...
    def __init__(self, name, solverTypeName, extension, metaData=None):
        ...
        bs.loadCoreClient()
        bs.loadExtension('Beam')
        if self.extension != 'Beam':
            bs.loadExtension(self.extension)
        self.solverRTVal = bs.constructRTVal('%s::%s' % (self.extension, self.solverTypeName))

        # logger.debug("Creating kl operator object [%s] of type [%s] from extension [%s]:" % (self.getName(), self.solverTypeName, self.extension))

        self.args = self.solverRTVal.getArguments('Beam:KrakenSolverArg[]')
    ...
    def getDefaultValue(self, name, RTValDataType, mode="arg"):
        ...
        if self.solverRTVal.defaultValues.has("Boolean", name).getSimpleType():
            ...
            RTVal = bs.convertFromRTVal(self.solverRTVal.defaultValues[name])
            ...
            if RTVal.isArray():
                ...
                if isFixedArrayType(RTVal.getTypeName().getSimpleType()) and not isFixedArrayType(RTValDataType):
                    RTValArray = bs.rtVal(RTValDataType)
                    if len(RTVal):
                ...
                RTVal = bs.convertFromRTVal(self.solverRTVal.defaultValues[name], RTTypeName=RTValDataType)

            logger.debug("Using default value for %s.%s.%s(%s) --> %s" % (self.solverTypeName, self.getName(), mode, name, RTVal))

            return RTVal

        defaultValue = bs.rtVal(RTValDataType)
    ...
    def evaluate(self):
        ...
        def getRTVal(obj, asInput=True):
            if isinstance(obj, Object3D):
                ...
                return bs.rtVal('Boolean', obj)
            elif type(obj) is int:
                return bs.rtVal('Integer', obj)
            elif type(obj) is float:
                return bs.rtVal('Scalar', obj)
            elif type(obj) is str:
                return bs.rtVal('String', obj)
            else:
                return obj #
        ...
        for i in xrange(len(self.args)):
            ...
            if argDataType == 'EvalContext':
                argVals.append(bs.constructRTVal(argDataType))
                continue
            if argName == 'time':
                argVals.append(bs.constructRTVal(argDataType))
                continue
            ...
            if argConnectionType == 'In':
                if str(argDataType).endswith('[]'):
                    if argName in self.inputs and self.inputs[argName] is not None:
                        rtValArray = bs.rtVal(argDataType)
