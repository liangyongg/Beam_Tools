...
from rigging.beam.core.maths import Vec2, Vec3, Xfo, Mat44, Math_radToDeg, RotationOrder
import rigging.beam.core.builder
reload(rigging.beam.core.builder)
from rigging.beam.core.builder import Builder
...
from rigging.beam.helpers.utility_methods import prepareToSave, prepareToLoad
...
class Builder(Builder):
    ...
    def buildContainer(self, kSceneItem, buildName):
        ...
        if kSceneItem.isTypeOf('Rig'):
            beamRigAttr = dccSceneItem.addAttr('beamRig',
                                                 niceName='beamRig',
                                                 attributeType="bool",
                                                 defaultValue=True,
                                                 keyable=False)

            dccSceneItem.attr('beamRig').setLocked(True)
            ...
            if 'guideData' in metaData:
                ...
                beamRigDataAttr = dccSceneItem.addAttr('beamRigData',
                                                     niceName='beamRigData',
                                                     dataType="string",
                                                     keyable=False)

                dccSceneItem.attr('beamRigData').set(json.dumps(pureJSON, indent=2))
                dccSceneItem.attr('beamRigData').setLocked(True)

        self._registerSceneItemPair(kSceneItem, dccSceneItem)
        print "builder.buildContainer.parentNode: ", parentNode
        print "builder.buildContainer.dccSceneItem: ", dccSceneItem
        print "builder.buildContainer.kSceneItem.getTypeName: ",kSceneItem.getTypeName()
        print "builder.buildContainer.metaData: ", metaData
        return dccSceneItem
    ...
    def buildLayer(self, kSceneItem, buildName):
        ...
        print "builder.buildLayer.parentNode: ", parentNode
        print "builder.buildLayer.dccSceneItem: ", dccSceneItem
        print "builder.buildLayer.kSceneItem.getTypeName: ",kSceneItem.getTypeName()
        return dccSceneItem
    ...
    def buildCanvasOperator(self, kOperator, buildName, isKLBased=False):
        ...
        try:
            if isKLBased is False:
                host = bs.getCoreClient().DFG.host
