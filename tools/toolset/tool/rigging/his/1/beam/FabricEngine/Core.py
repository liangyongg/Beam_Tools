...
def invalidateSingletonClient():
  CAPI.Client.InvalidateSingleton()
...
class _FabricClient(object):
  ...
  def __init__(self, opts):
    ...
    if opts is not None and 'reportCallback' in opts:
      ...
      def reportCallback(source, level, line):
        if source == ReportSource.System:
          sys.stderr.write(line+ "\n")
        if source == ReportSource.User:
    ...
    if opts is not None and 'contextID' in opts:
      ...
      createOptions['trapOnThrow'] = 0
      if opts is not None and 'trapOnThrow' in opts:
        createOptions['trapOnThrow'] = bool(opts['trapOnThrow'])
      ...
      if opts is not None and 'exts' in opts:
        if isinstance(opts['exts'], dict):
          ...
          raise "opts['extsToLoad']: must be a list or a dict"
      ...
      createOptions['canvasPresetDirs'] = []
      if opts is not None and 'canvasPresetDirs' in opts:
        for extPath in opts['canvasPresetDirs']:
          createOptions['canvasPresetDirs'].append(extPath)
      ...
      createOptions['rtValToJSONEncoder'] = None
      if opts is not None and 'rtValToJSONEncoder' in opts:
        createOptions['rtValToJSONEncoder'] = opts['rtValToJSONEncoder']
      
      createOptions['rtValFromJSONDecoder'] = None
      if opts is not None and 'rtValFromJSONDecoder' in opts:
        createOptions['rtValFromJSONDecoder'] = opts['rtValFromJSONDecoder']

      createOptions['interactive'] = 0
      if opts is not None and 'interactive' in opts:
        createOptions['interactive'] = bool(opts['interactive'])
      
      createOptions['singleton'] = False
      if opts is not None and 'singleton' in opts:
        createOptions['singleton'] = bool(opts['singleton'])
    ...
    self.userData = {}
    self.DFG = _DFG()
    self.DFG.host = self.getDFGHost()
    self.DFG.host.PortTypes = self.DFG.PortTypes
    self.DFG.host.CacheRule = self.DFG.CacheRule
  ...
  def loadExtension(self, extension, versionRange="", reloadExt=False):
    return self._client.loadExtension(extension, versionRange, reloadExt)
    
  def getExtensionsDesc(self):
    return self._client.getExtensionsDesc()
  ...
  RegisterKLExtension_Flag_AutoNamespace = CAPI.RegisterKLExtension_Flag_AutoNamespace
  RegisterKLExtension_Flags_Default = CAPI.RegisterKLExtension_Flags_Default

  def registerKLExtension(
    self,
    name,
    sourceFiles,
    version="0.0.0",
    ...
    loadExt=True,
    reloadExt=False,
    flags=CAPI.RegisterKLExtension_Flags_Default,
    ):
    CAPI.RegisterKLExtensionEx(
      self._client,
      name,
      version,
      override,
  ...
  ExportKLExtensions_Flag_RequireAutoNamespace = CAPI.ExportKLExtensions_Flag_RequireAutoNamespace
  ExportKLExtensions_Flags_Default = CAPI.ExportKLExtensions_Flags_Default

  def exportKLExtensions(
    self,
    extNames,
    suffix,
    flags=CAPI.ExportKLExtensions_Flags_Default,
    ):
    return CAPI.ExportKLExtensions(
      self._client,
      extNames,
      suffix,
      flags,
  ...
  def importKLExtensions(
    self,
    extsJSON,
    ):
    return CAPI.ImportKLExtensions(
      self._client,
      extsJSON,
      )

  def getKLJSONAST(self, fileName, sourceFile, includeRequires):
    return CAPI.GetKLJSONAST(self._client, fileName, sourceFile, includeRequires)
...
class _RT(object):
  ...
  def getRegisteredTypeIsStruct(self, name):
    return CAPI.GetRegisteredTypeIsStruct(self._client, name) 
  
  def getRegisteredTypeIsObject(self, name):
    return CAPI.GetRegisteredTypeIsObject(self._client, name) 
  
  def getRegisteredTypeIsInterface(self, name):
    return CAPI.GetRegisteredTypeIsInterface(self._client, name) 
  
  def getRegisteredTypeExtName(self, name):
    return CAPI.GetRegisteredTypeExtName(self._client, name) 
  
  def constructRTValFromJSON(self, typeNameString, jsonString):
    return Util.rtValToPyObject(
      self._client.getContext(), 
      CAPI.ConstructRTValFromJSON(self._client, typeNameString, jsonString)
      )
...
class _Build(object):
  ...
  def getCopyright(self):
    return "Copyright (c) 2010-2017 Fabric Software Inc. All rights reserved."
