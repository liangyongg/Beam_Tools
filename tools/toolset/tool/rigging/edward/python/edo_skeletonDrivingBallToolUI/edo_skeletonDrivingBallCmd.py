...
import edo_autoConnectBlendshapesInbetweenUI.edo_createFacialCtrl as edo_createFacialCtrl
import edo_skeletonDrivingBallToolUI.edo_createEightDirPlane as edo_createEightDirPlane
#reload(edo_createEightDirPlane)
import os

filepath=os.path.dirname(edo_skeletonDrivingBallToolUI.__file__).replace('\\','/')+'/file/scenes/SKDB_FRAME.mel'
...
def edo_mirrorSkeletonBallCmd():
    ...
    for sel in sels:
        ...
        if 'L_' in sel:
            edo_duplicateSetDrivenKey(sel,'L_','R_')
        if 'R_' in sel:
            edo_duplicateSetDrivenKey(sel,'R_','L_')
...
def edo_mirrorSkeletonBall(skball):
    ...
    if 'L_' in skball:
        ...
        mirrorjnt=skball.replace('L_','R_').replace('_FRAME','')
    ...
    if 'R_' in skball:
        ...
        ro=cmds.xform(skball,q=1,ws=1,ro=1)
        mro=[ro[0],ro[1]*-1,ro[2]*-1]
        mirrorjnt=skball.replace('R_','L_').replace('_FRAME','')
        if cmds.objExists(mirrorjnt):
            edo_addSkeletonBall(mirrorjnt)
            cmds.setAttr(mirrorjnt+'_FRAME.globalScale',gs)
            cmds.xform(mirrorjnt+'_FRAME',ws=1,ro=mro)
...
#edo_addSkeletonBall('joint2',1,None,None,8)
def edo_addSkeletonBall(jnt,autoConstraint=0,parents=None,rotation=None,axis=4):
    ...
    if axis==8:
        print '8 axis model...'
        cmds.addAttr(jnt+'_FRAME',ln='Axis_4_or_8',at='float',dv=1,minValue=0,maxValue=1)
        ...
        if not rotation==None:
            for r in rotation:
                ...
                if str(type(r))=="<type 'str'>":
                    if cmds.objExists(r):
                        cmds.delete(cmds.orientConstraint(r,jnt+'_FRAME',mo=0))
                else:
                    cmds.rotate(float(r[0]),float(r[1]),float(r[2]),jnt+'_FRAME',r=1,os=1)
        if not parents==None:
...
def edo_addSkeletonBallGlobalScaleAttr(framename):
    ...
    if cmds.attributeQuery('globalScale',n=framename,ex=1):
        if cmds.objExists(framename+'_globalScaleLoc'):
            cmds.delete(framename+'_globalScaleLoc')
