...
def edo_crateNewBlendShapeMeshToNewMeshWindow():
    ...
    cm.button('crateNewBlendShapeMeshToNewMeshBT1',label='load orgMesh',bgc=(0.9,0.7,0.1),c=partial(edo_checkBT1))
    cm.text( label='=====================================' )

    cm.button('crateNewBlendShapeMeshToNewMeshBT2',label='load newMesh',bgc=(0.9,0.7,0.1),c=partial(edo_checkBT2))
    cm.text( label='=====================================' )

    cm.text( label='and the last,please select the all blendShapeMesh')
    cm.button('crateNewBlendShapeMeshToNewMeshBT3',label='start',bgc=(0.5,0.8,1),c=partial(edo_crateNewBlendShapeMeshToNewMesh))
...
def edo_crateNewBlendShapeMeshToNewMesh(*arg):
    global edo_crateNewBlendShapeMeshToNewMesh_org
    global edo_crateNewBlendShapeMeshToNewMesh_new
    allbs=cm.ls(sl=1);
    if len(allbs)>0:
